...
class YesTokenAuth
{
    ...
    public function verifyToken($encryptedToken = null)
    {
        ...
        try {
            ...
            $decoded = FirebaseJwt::decode($decryptedToken, $this->key, ['HS256']);
            ...
            if ($this->useTokenRegistry == true) {
                $isRegistred = $this->tokenRegistryRepository->fetch($decoded->jti);

                if (__isEmpty($isRegistred)) {
                    return [
                        'error' => 'token not registred',
                    ];
                }

                if ($decryptedToken !== $isRegistred->{config('yes-token-auth.token_registry.schema.jwt_token', 'jwt_token')}) {
                    return [
                        'error' => 'token mismatched',
                    ];
                    // if($decoded->jti !== $isRegistred->predecessor_token_id) {
                    //     return [
                    //         'error' => 'token mismatched'
                    //     ];
                    // }
            ...
            // cleanup registry by deleteing token not in use or expired
